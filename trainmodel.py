...
Original file is located at
    https://colab.research.google.com/drive/1DJQyrUIQzOzbHgWT8tuzqpKwIUzVYz6I
"""

# %%capture
# !unzip /content/drive/MyDrive/Colab_Notebooks/Test_task/data/airbus-ship-detection.zip -d /content/data
...
masks = pd.read_csv('/content/data/train_ship_segmentations_v2.csv')
...
def load_img(c_img_id):
  try:
    return os.stat(os.path.join(train_image_dir, c_img_id)).st_size/1024
  except:
    pass

unique_img_ids['file_size_kb'] = unique_img_ids['ImageId'].map(load_img) 
...
weight_path="{}_weights.best_2.hdf5".format('seg_model')
...
def fit():
    seg_model.compile(optimizer=Adam(1e-3, decay=1e-6), loss=gdice_coef_loss, metrics=['binary_accuracy', gdice_coef])
...
def show_loss(loss_history):
    ...
    _ = ax3.plot(epochs, np.concatenate([mh.history['gdice_coef'] for mh in loss_history]), 'b-',
                 epochs, np.concatenate([mh.history['val_gdice_coef'] for mh in loss_history]), 'r-')
...
seg_model.save('seg_model_weights.best.h5')
